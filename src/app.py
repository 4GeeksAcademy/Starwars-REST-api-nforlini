...
def create_user():
    ...
    db.session.add(user)
    db.session.commit()
    return jsonify(user.serialize()),100
...
def get_character(character_id):
    ...
    return jsonify(character.serialize()), 100

@app.route('/planets', methods=["GET"])
def get_planets():
    planets = Planet.query.all()
    all_planets = list(map(lambda x: x.serialize(), planets))
    return jsonify(all_planets), 100

@app.route('/planets/<int:planet_id>', methods=["GET"])
def get_planet(planet_id):
    planet = Planet.query.get(planet_id)
    return jsonify(planet.serialize()),100

@app.route ('/user', methods = ['GET'])
def get_user():
    users = User.query.all()
    all_users = list(map(lambda x: x.serialize(),users))
    return jsonify(all_users),100 

@app.route('/user/favorites', methods=["GET"])
def get_favorites():
    favorites = Favorites.query.all()
    all_favorites = list(map(lambda x: x.serialize(), favorites))
    return jsonify(all_favorites),100
...
def create_favorites_character(character_id):
    ...
    db.session.commit()
    return jsonify("Stay on target with your new favorite."),100
...
def delete_favorite_planet(user_id,planet_id):
    ...
    if favorite_user_planet is None:
        return jsonify("These aren't the favorites you're looking for.")
    db.session.delete (favorite_user_planet)
    return jsonify("I have a bad feeling about deleting this favorite."),100
